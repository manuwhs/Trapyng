...
mySymbol_GE = myPortfolio.get_symbols(["GE"])[0] # Obtain one of the symbols
myPortfolio.del_symbols(["XOM"])

# Create an independent symbol and add it to the portfolio
mySymbol_HPQ = CSy.CSymbol("HPQ",periods) 
mySymbol_HPQ.set_csv(storage_folder)
myPortfolio.add_symbols([mySymbol_HPQ])
...
close_some = myPortfolio.get_timeSeries(symbolIDs = ["GE","DANSKE.CO"])
...
ATR = myPortfolio.ATR(symbolIDs = ["GE","DANSKE.CO"],n = 10)
