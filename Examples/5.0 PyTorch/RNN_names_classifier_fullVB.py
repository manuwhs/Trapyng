...
def lineToTensor(line):
    ...
    for li, letter in enumerate(line):
        ...
        return tensor
class RNN_names_classifier_fullVB(GeneralVBModelRNN):
    """
    ...
    def __init__(self, conf_a, prior):
        ...
        self.lstm1 = LSTMCellVB(conf_a.D_in, conf_a.HS, bias = True, prior = prior).to(device=conf_a.device, dtype=conf_a.dtype)
...
#        self.linear2 = LinearVB(in_features = 10, out_features = 1, bias=True, prior = prior).to(device=conf_a.device, dtype=conf_a.dtype)
        ...
        """
        self.VBmodels = [self.linear, self.lstm1]
        self.LinearModels = []
...
#        print ("predictions dim ", outputs.shape)
        return outputs
    """
    FUNCTIONS FOR THE SPECIFIC PROBLEM !!
    """
