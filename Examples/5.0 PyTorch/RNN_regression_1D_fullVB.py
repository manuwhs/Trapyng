...
class RNN_regression_1D_fullVB(GeneralVBModel):
    """
...
#        super(GeneralVBModelRNN, self).__init__()
        ...
        self.lstm1 = LSTMCellVB(1, conf_a.HS,True, prior).to(device=conf_a.device, dtype=conf_a.dtype)
#        self.lstm2 = LSTMCellVB(conf_a.HS, conf_a.HS,prior).to(device=conf_a.device, dtype=conf_a.dtype)
...
#        self.linear2 = LinearVB(in_features = 10, out_features = 1, bias=True, prior = prior).to(device=conf_a.device, dtype=conf_a.dtype)
        ...
        """
        self.VBmodels = [ self.lstm1 , self.linear]
        self.LinearModels = []
