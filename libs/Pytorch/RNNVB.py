    ...
    def __init__(self, mode, input_size, hidden_size,
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                ...
                ################ STDS ##############3
                rho_w_ih = Parameter(torch.Tensor(gate_size, layer_input_size))
                rho_w_hh = Parameter(torch.Tensor(gate_size, hidden_size))
                rho_b_ih = Parameter(torch.Tensor(gate_size))
                rho_b_hh = Parameter(torch.Tensor(gate_size))
